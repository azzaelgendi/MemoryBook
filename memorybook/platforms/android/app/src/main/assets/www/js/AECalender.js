/**
 * File Name: AECalender.js
 *
 * Revision History:
 *       Azza Elgendy, 2019-04-08 : Created
 */
...
var calendarName = 'My Memory Book Events';
var options = {
...
function addToCalander() {
    if (AEdoValidate_AEEditForm()) {
        title = $("#AEEditTitle").val();
        loc = $("#AEEditLoc").val();
        notes = $("#AEEditNotes").val();
        // startDate = $("#AEEditmemoryDate").val();

        createCalendarEventInteractivelyWithOptions(title, location, notes, startDate, endDate, options, onSuccess, onError);
    }
    else {
        alert("Adding to the calender failed");
    }
